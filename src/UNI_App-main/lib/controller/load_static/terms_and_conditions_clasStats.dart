...
Future<String> readTermsAndConditionsClasStats() async {
  if (await (Connectivity().checkConnectivity()) != ConnectionState.none) {
    //Uncomment this in order the ReadMe text from a server
    //try {
  ...
  try {
    return await rootBundle.loadString('assets/text/TermsAndConditionsClasStats.md');
  } catch (e) {
    Logger().e('Failed to read Terms and Conditions: ${e.toString()}');
    return 'Failed to fetch Terms and Conditions. '
...
Future<bool> updateTermsAndConditionsAcceptancePreference() async {
  ...
  final termsAndConditions = await readTermsAndConditionsClasStats();
